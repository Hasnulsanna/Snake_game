...
while True:
    ...
    # Using numpy arange function for better performance. Loop until all detected points
    for i in np.arange(1, len(pts)):
        # If no points are detected, move on.
        if (i - 10 >= 0 and pts[i - 10] is not None) and (pts[i - 1] is not None and pts[i] is not None):
            ...
            if counter >= 10:
                ...
                # If distance is greater than 50 pixels, considerable direction change has occurred.
                if np.abs(dX) > 50:
